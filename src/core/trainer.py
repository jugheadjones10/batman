...
# Detection model sizes supported by the application.
_RFDETR_CLASS_TABLE = {
    "nano": "RFDETRNano",
    "small": "RFDETRSmall",
    "base": "RFDETRBase",
...
def resolve_rfdetr_class(model_size: str):
    """Return the detection model class, rejecting unsupported sizes."""
    size = model_size.lower()
    if size not in _RFDETR_CLASS_TABLE:
        raise ValueError(f"Unsupported detection model size: {model_size!r}")
    import rfdetr

    return getattr(rfdetr, _RFDETR_CLASS_TABLE[size])


def validate_detection_checkpoint(checkpoint_path: Path) -> None:
    """Reject legacy non-detection runs before loading their weights."""
    checkpoint_path = Path(checkpoint_path)
    for directory in (checkpoint_path.parent, checkpoint_path.parent.parent):
        for filename in ("class_info.json", "meta.json"):
            path = directory / filename
            if not path.exists():
                continue
            with path.open() as f:
                data = json.load(f)
            training = (data.get("config") or {}).get("training") or {}
            tasks = (data.get("task"), training.get("task"))
            if any(task and task != "detection" for task in tasks):
                raise ValueError("Only detection checkpoints are supported; retrain this run for detection.")
...
class RFDETRTrainer:
    ...
    def __init__(
        ...
        Args:
            model_size: Model size ('nano', 'small', 'base', 'medium', or 'large')
    ...
    def _load_model(self):
        """Load the RF-DETR model for the configured size."""
        Model = resolve_rfdetr_class(self.model_size)
        if self.checkpoint:
    ...
    def train(
        ...
        resolution = max(280, (config.image_size // block_size) * block_size)
        if config.resume:
            validate_detection_checkpoint(Path(config.resume))
...
def _load_rfdetr_model(
    ...
    """Instantiate the correct RF-DETR variant for the given size."""
